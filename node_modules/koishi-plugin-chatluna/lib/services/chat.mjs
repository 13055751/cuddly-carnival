...
var DefaultChatChainSender = class {
  ...
  convertToForwardMessages(messages) {
    ...
    if (Array.isArray(firstMsg)) {
      return messages.map((msg) => h("message", ...msg));
    }
    if (typeof firstMsg === "object") {
      return [h("message", ...messages)];
    }
...
import { h as h2, Schema } from "koishi";
import he from "he";
var TextRenderer = class extends Renderer {
  ...
  async render(message, options) {
    ...
    if (transformed[0]?.type === "p") {
      ...
      if (pElementContent) {
        transformed.unshift(h2.text(pElementContent));
      } else {
...
import { h as h3, Schema as Schema2 } from "koishi";
var VoiceRenderer = class extends Renderer {
  ...
  async render(message, options) {
    ...
    logger3?.debug(`splitMessages: ${JSON.stringify(splitMessages)}`);
    if (options.split) {
      return {
  ...
  schema = Schema2.const("voice").i18n({
    "zh-CN": "将回复渲染为语音",
...
// src/renders/raw.ts
import { h as h4, Schema as Schema3 } from "koishi";
var RawRenderer = class extends Renderer {
  ...
  async render(message, options) {
    return {
      element: h4.text(message.content)
    };
  }
  schema = Schema3.const("raw").i18n({
    "zh-CN": "原始输出",
    "en-US": "Raw text"
  });
};
...
import { h as h5, Schema as Schema4 } from "koishi";
import he2 from "he";
...
var KoishiElementRenderer = class extends Renderer {
  ...
  schema = Schema4.const("koishi-element").i18n({
...
function transformAndEscape2(source) {
  try {
    return h5.parse(source).map(unescape2);
  } catch (e) {
    logger4.error(e);
    return [h5.text(source)];
  }
}
...
import { h as h6, Schema as Schema5 } from "koishi";
var MixedVoiceRenderer = class extends Renderer {
  ...
  async renderVoice(message, options) {
    const splitMessages = this._splitMessage(message.content).flatMap((text) => text.trim().split("\n\n")).filter((text) => text.length > 0);
    logger5?.debug(`splitMessages: ${JSON.stringify(splitMessages)}`);
    return {
  ...
  schema = Schema5.const("mixed-voice").i18n({
    "zh-CN": "同时输出语音和文本",
...
var DefaultRenderer = class {
  ...
  async render(message, options = this.defaultOptions) {
    try {
      ...
      throw new ChatLunaError3(ChatLunaErrorCode3.RENDER_ERROR, e);
    }
  }
...
import { Time as Time2 } from "koishi";
...
var PresetFormatService = class {
  ...
  _initBuiltinFunctions() {
    this._functionHandlers["time_UTC"] = (args) => {
      ...
      if (isNaN(utcOffset)) {
        logger6.warn(`Invalid UTC offset: ${args[0]}`);
        return "Invalid UTC offset";
      }
      const offsetDate = new Date(+date + utcOffset * Time2.hour);
    ...
    this._functionHandlers["date"] = () => {
      ...
      const offsetDate = new Date(
        +date - date.getTimezoneOffset() * Time2.minute
      );
  ...
  replaceFunction(name, handler) {
    if (this._functionHandlers[name] == null) {
      logger6.warn(`Function handler for ${name} not exists.`);
    }
    this._functionHandlers[name] = handler;
    return () => {
      delete this._functionHandlers[name];
      ...
      tokens.map(async (token) => {
        switch (token.type) {
          ...
          case "variable": {
            ...
            if (!result) {
              result = "";
              logger6.warn(`Variable ${token.value} not found`);
            }
            return result;
          }
          case "function": {
...
var ChatLunaService = class extends Service {
  ...
  async registerPlugin(plugin) {
    ...
    if (this._plugins[platformName]) {
      throw new ChatLunaError4(
        ChatLunaErrorCode4.PLUGIN_ALREADY_REGISTERED,
  ...
  async awaitLoadPlatform(plugin, timeout = 3e4) {
    ...
    if (this._platformService.getModels(pluginName, ModelType.all).length > 0) {
      resolve();
      return promise;
    }
  ...
  chat(session, room, message, event, stream = false, variables = {}, postHandler, requestId = uuidv4()) {
  ...
  async createChatModel(platformName, model) {
    ...
    if (client == null) {
      throw new ChatLunaError4(
        ChatLunaErrorCode4.MODEL_ADAPTER_NOT_FOUND,
...
var ChatLunaPlugin = class {
  constructor(ctx, config, platformName, createConfigPool = true) {
    ...
    if (createConfigPool) {
      this._platformConfigPool = new ClientConfigPool(
        ctx,
        config.configMode === "default" ? ClientConfigPoolMode.AlwaysTheSame : ClientConfigPoolMode.LoadBalancing
      );
    }
  ...
  async initClients() {
    ...
    this._supportModels = this._supportModels.concat(
      this._platformService.getModels(this.platformName, ModelType.llm).map((model) => `${this.platformName}/${model.name}`)
    );
  }
  async initClientsWithPool(platformName, pool, createConfigFunc) {
    ...
    this._supportModels = this._supportModels.concat(
      this._platformService.getModels(platformName, ModelType.llm).map((model) => `${platformName}/${model.name}`)
    );
  }
  get supportedModels() {
