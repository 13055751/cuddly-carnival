...
__export(model_exports, {
  ChatHubBaseEmbeddings: () => ChatHubBaseEmbeddings,
...
async function getEncoding(encoding, options) {
  ...
  if (cache[encoding]) {
    return new import_lite.Tiktoken(cache[encoding], options?.extendedSpecialTokens);
  }
...
function formatFunctionDefinitions(functions) {
  ...
  for (const f of functions) {
    ...
    const schema = (0, import_zod_to_json_schema.zodToJsonSchema)(f.schema);
...
var ChatLunaChatModel = class extends import_chat_models.BaseChatModel {
  ...
  async _withTimeout(func, timeout) {
    ...
    const timeoutId = setTimeout(() => {
      reject(new import_error.ChatLunaError(import_error.ChatLunaErrorCode.API_REQUEST_TIMEOUT));
    }, timeout);
    ...
    if (!this.__encoding) {
      try {
        this.__encoding = await encodingForModel(
          "modelName" in this ? (0, import_count_tokens.getModelNameForTiktoken)(this.modelName) : "gpt2"
        );
      } catch (error) {
      }
    }
...
var ChatHubBaseEmbeddings = class extends import_embeddings.Embeddings {
  static {
