...
export interface ChatLunaChainInfo {
    ...
    createFunction: (params: CreateChatLunaLLMChainParams) => Promise<ChatLunaLLMChainWrapper>;
}
...
export interface ChatLunaTool {
    createTool: (params: CreateToolParams, session?: Session) => Promise<StructuredTool>;
    selector: (history: BaseMessage[]) => boolean;
    authorization?: (session: Session) => boolean;
    alwaysRecreate?: boolean;
}
export type CreateVectorStoreFunction = (params: CreateVectorStoreParams) => Promise<ChatLunaSaveableVectorStore>;
