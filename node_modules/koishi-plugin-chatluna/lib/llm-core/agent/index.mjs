...
Passed input: ${text}`
  ...
  parseAIMessage(message) {
    ...
    if ((message instanceof AIMessageChunk || message instanceof AIMessage) && message.tool_calls != null) {
...
function createOpenAIAgent({
  ...
  const agent = RunnableSequence.from([
    ...
    RunnableLambda.from((input) => {
      if ((input?.additional_kwargs?.tool_calls || (input instanceof AIMessageChunk2 || input instanceof AIMessage2) && input.tool_calls) && outputParser instanceof OpenAIFunctionsAgentOutputParser) {
...
var AgentRunnableSequence = class extends RunnableSequence2 {
  static {
  ...
  static fromRunnables([first, ...runnables], config) {
    const sequence = RunnableSequence2.from(
      [first, ...runnables],
      config.name
    );
...
import {
  patchConfig as patchConfig2,
  Runnable as Runnable2
} from "@langchain/core/runnables";
import {
  CallbackManager
...
import { Serializable as Serializable2 } from "@langchain/core/load/serializable";
var AgentExecutorIterator = class extends Serializable2 {
  static {
...
var AgentExecutor = class _AgentExecutor extends BaseChain {
  ...
  constructor(input) {
    ...
    if (Runnable2.isRunnable(input.agent)) {
...
import { RunnablePassthrough as RunnablePassthrough2 } from "@langchain/core/runnables";

// src/llm-core/agent/react/output_parser.ts
import { renderTemplate } from "@langchain/core/prompts";
import { OutputParserException as OutputParserException3 } from "@langchain/core/output_parsers";
...
// src/llm-core/agent/react/index.ts
import { AgentRunnableSequence as AgentRunnableSequence2 } from "koishi-plugin-chatluna/llm-core/agent";
...
async function createReactAgent({
  ...
  const agent = AgentRunnableSequence2.fromRunnables(
    [
