...
var ChatHubChatChain = class _ChatHubChatChain extends import_base.ChatLunaLLMChainWrapper {
  static {
...
var ChatLunaPluginChain = class _ChatLunaPluginChain extends import_base2.ChatLunaLLMChainWrapper {
  ...
  async _createExecutor(llm, tools) {
    if (this.agentMode === "react") {
      return import_agent.AgentExecutor.fromAgentAndTools({
        tags: ["react"],
    ...
    return import_agent.AgentExecutor.fromAgentAndTools({
      tags: ["tool-calling"],
      agent: (0, import_agent.createOpenAIAgent)({
        llm,
        tools,
        prompt: this.prompt
      }),
  ...
  async call({
    ...
    for (let i = 0; i < 3; i++) {
      if (signal.aborted) {
        throw new import_error2.ChatLunaError(import_error2.ChatLunaErrorCode.ABORTED);
      }
      try {
    ...
    response.message = new import_messages2.AIMessage(responseString);
    return response;
  }
  get model() {
    return this.llm;
...
async function defaultFactory(ctx, service) {
  ...
  ctx.on("chatluna/tool-updated", () => {
    ...
    wrapper.getCachedConversations().filter(
      ...
      if (result2) {
        import_koishi_plugin_chatluna.logger?.debug(`Cleared cache for room ${id}`);
      }
    });
  });
...
async function queryJoinedConversationRoom(ctx, session, name2) {
  ...
  if (userRoomInfoList.length > 1) {
    throw new import_error4.ChatLunaError(
      import_error4.ChatLunaErrorCode.UNKNOWN_ERROR,
...
async function getTemplateConversationRoom(ctx, config) {
  const selectModelAndPreset = /* @__PURE__ */ __name(async () => {
    if (config.defaultModel === "无" || config.defaultModel == null) {
      const models = ctx.chatluna.platform.getAllModels(import_types2.ModelType.llm);
  ...
  if (config.defaultChatMode == null || config.defaultModel === "无" || config.defaultPreset == null) {
    if (config.defaultChatMode == null) {
      throw new import_error4.ChatLunaError(import_error4.ChatLunaErrorCode.ROOM_TEMPLATE_INVALID);
    }
...
async function transferConversationRoom(ctx, session, room, userId) {
  ...
  if (memberList.length === 0) {
    throw new import_error4.ChatLunaError(import_error4.ChatLunaErrorCode.ROOM_NOT_FOUND);
  }
  ...
  if (oldMaster.length === 1) {
    ...
    throw new import_error4.ChatLunaError(import_error4.ChatLunaErrorCode.ROOM_NOT_FOUND_MASTER);
  }
  await ctx.database.upsert("chathub_room_member", [
...
async function switchConversationRoom(ctx, session, id) {
  ...
  if (joinedRoom.length > 1) {
    throw new import_error4.ChatLunaError(
      import_error4.ChatLunaErrorCode.THE_NAME_FIND_IN_MULTIPLE_ROOMS
    );
...
async function muteUserFromConversationRoom(ctx, session, roomId, userId) {
  ...
  if (memberList.length === 0) {
    throw new import_error4.ChatLunaError(import_error4.ChatLunaErrorCode.ROOM_NOT_JOINED);
  }
  await ctx.database.upsert("chathub_room_member", [
...
function apply16(ctx, config, chain) {
  ...
  chain.middleware("chat_time_limit_check", async (session, context) => {
    ...
    let authGroup = await authService.resolveAuthGroup(
      session,
      (0, import_count_tokens2.parseRawModelName)(model)[0]
    );
  ...
  async function oldChatLimitCheck(session, context) {
    ...
    if (!config2) {
      throw new import_error5.ChatLunaError(
        import_error5.ChatLunaErrorCode.MODEL_ADAPTER_NOT_FOUND,
    ...
    key = import_crypto2.default.createHash("md5").update(key).digest("hex");
...
function apply17(ctx, config, chain) {
  ...
  async function oldChatLimitSave(session, context) {
    ...
    let key = conversationId + "-" + session.userId;
    key = import_crypto3.default.createHash("md5").update(key).digest("hex");
    chatLimit.count++;
...
function apply21(ctx, config, chain) {
  chain.middleware("clone_preset", async (session, context) => {
    ...
    const loaded = (0, import_js_yaml2.load)(oldPreset.rawText);
...
function checkModelList(service, models) {
  const availableModels = service.getAllModels(import_types3.ModelType.llm);
...
function apply24(ctx, config, chain) {
  ...
  chain.middleware("create_room", async (session, context) => {
    ...
    while (true) {
      ...
      const findModel = service.getAllModels(import_types4.ModelType.llm).find((searchModel) => searchModel === preModel);
      if (findModel == null) {
        await context.send(
...
async function createRoom(ctx, context, session, options) {
  ...
  const createRoom2 = {
    conversationId: (0, import_crypto4.randomUUID)(),
    model,
    preset,
...
function apply25(ctx, config, chain) {
  logger5 = (0, import_logger3.createLogger)(ctx);
...
function apply32(ctx, config, chain) {
  ...
  chain.middleware("list_all_embeddings", async (session, context) => {
    ...
    const models = service.getAllModels(import_types5.ModelType.embeddings);
...
function apply33(ctx, config, chain) {
  ...
  chain.middleware("list_all_model", async (session, context) => {
    ...
    const models = services.getAllModels(import_types6.ModelType.llm);
    await pagination.push(models);
    context.message = await pagination.getFormattedPage(page, limit);
    return 1 /* STOP */;
  }).after("lifecycle-handle_command");
...
function apply38(ctx, config, chain) {
  logger6 = (0, import_logger4.createLogger)(ctx);
  chain.middleware("message_delay", async (session, context) => {
    ...
    const lock = queueLock[room.conversationId] || new import_lock.ObjectLock();
    ...
    unlock();
    const { promise, resolve } = (0, import_promise.withResolver)();
...
var BufferText = class {
  ...
  queue = [];
  currentText = "";
  lock = new import_lock2.ObjectLock();
  isEnd = false;
  isTextStarted = false;
  rawText = "";
  async addText(text) {
...
function apply43(ctx, config, chain) {
  logger7 = (0, import_logger5.createLogger)(ctx);
        ...
        inputMessage,
        {
          ...
          "llm-used-token-count": /* @__PURE__ */ __name(async (tokens) => {
            ...
            const balance = await ctx.chatluna_auth.calculateBalance(
              session,
              (0, import_count_tokens3.parseRawModelName)(room.model)[0],
              tokens
            );
...
function apply45(ctx, config, chain) {
  logger8 = (0, import_logger6.createLogger)(ctx);
  chain.middleware("resolve_room", async (session, context) => {
    ...
    if (joinRoom == null && (context.command?.length ?? 0) < 1) {
      ...
      cloneRoom.conversationId = (0, import_uuid2.v4)();
...
function apply50(ctx, config, chain) {
  ...
  chain.middleware("search_model", async (session, context) => {
    ...
    const models = services.getAllModels(import_types7.ModelType.llm);
    await pagination.push(models);
...
function checkModelList2(service, models) {
  const availableModels = service.getAllModels(import_types8.ModelType.llm);
  return models.some((model) => !availableModels.includes(model));
}
...
function apply54(ctx, config, chain) {
  ...
  chain.middleware("set_default_embeddings", async (session, context) => {
    ...
    const embeddings = service.getAllModels(import_types9.ModelType.embeddings);
    const [, modelName] = (0, import_count_tokens4.parseRawModelName)(setEmbeddings);
...
function apply57(ctx, config, chain) {
  ...
  chain.middleware("set_room", async (session, context) => {
    ...
    while (true) {
      ...
      const findModel = service.getAllModels(import_types10.ModelType.llm).find((searchModel) => searchModel === result3);
      if (findModel == null) {
        await context.send(
...
async function checkRoomAvailability(context, session, ctx, room) {
  ...
  const findModel = ctx.chatluna.platform.getAllModels(import_types10.ModelType.llm).find((searchModel) => searchModel === room.model);
  if (findModel == null) {
...
function apply61(ctx, config, chain) {
  logger9 = (0, import_logger7.createLogger)(ctx);
...
function apply63(ctx, config, chain) {
  logger10 = (0, import_logger8.createLogger)(ctx);
  chain.middleware("wipe", async (session, context) => {
    ...
    try {
      await import_promises5.default.rm("data/chathub/temp", { recursive: true });
    } catch (e) {
      logger10.warn(`wipe: ${e}`);
    }
    context.message = session.text(".success");
...
var Config2 = import_koishi13.Schema.intersect([
  ...
  import_koishi13.Schema.object({
    messageCount: import_koishi13.Schema.number().role("slider").min(2).max(500).step(1).default(40),
...
var VoiceRenderer = class extends Renderer {
  ...
  async render(message, options) {
    ...
    import_koishi_plugin_chatluna3.logger?.debug(`splitMessages: ${JSON.stringify(splitMessages)}`);
    if (options.split) {
      return {
...
function apply64(ctx, config) {
  logger = (0, import_logger9.createLogger)(ctx);
  setupLogger(config);
  setupI18n(ctx);
  const disposables = [];
  ctx.on("ready", async () => {
