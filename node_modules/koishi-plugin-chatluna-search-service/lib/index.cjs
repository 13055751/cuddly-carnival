...
var ChatLunaBrowsingChain = class _ChatLunaBrowsingChain extends import_base.ChatLunaLLMChainWrapper {
  static {
  ...
  constructor({
    ...
    this.responsePrompt = import_prompts.PromptTemplate.fromTemplate(searchPrompt);
    this.chain = chain;
    this.tools = tools;
  ...
  static fromLLMAndTools(llm, tools, {
    ...
    const formatQuestionChain = new import_base.ChatLunaLLMChain({
      llm: summaryModel,
      prompt: import_prompts.PromptTemplate.fromTemplate(newQuestionPrompt)
    });
    const contextualCompressionChain = contextualCompressionPrompt ? new import_base.ChatLunaLLMChain({
      llm: summaryModel,
      prompt: import_prompts.PromptTemplate.fromTemplate(
        contextualCompressionPrompt
      )
    }) : void 0;
  ...
  async call({
    ...
    const newQuestion = (await (0, import_base.callChatLunaChain)(
    ...
    const finalResponse = await (0, import_base.callChatLunaChain)(
      this.chain,
      {
        ...requests,
        stream,
    ...
    const aiMessage = finalResponse?.message ?? new import_messages.AIMessage(finalResponse.text);
    return {
      message: aiMessage
    };
  }
...
Content: ${action.content.join("\n")}`
      ...
      const rawSearchResults = await Promise.race([
        ...
        new Promise((resolve, reject) => {
          signal2?.addEventListener("abort", (event) => {
            reject(new import_error.ChatLunaError(import_error.ChatLunaErrorCode.ABORTED));
          });
        })
      ]);
    ...
    if (formattedSearchResults?.length > 0) {
      ...
      if (this.contextualCompressionChain) {
        try {
          formattedSearchResult = (await (0, import_base.callChatLunaChain)(
      ...
      chatHistory.push(new import_messages.SystemMessage(responsePrompt));
      chatHistory.push(
...
var PuppeteerBrowserTool = class extends import_tools.StructuredTool {
  static {
  ...
  schema = import_zod.z.object({
    action: import_zod.z.string().describe("The action to perform"),
    params: import_zod.z.string().optional().describe("The parameters for the action"),
    url: import_zod.z.string().optional().describe("The URL to action on")
  ...
  constructor(ctx, model, embeddings, options = {}) {
    ...
    this.pages = new import_lru_cache.LRUCache({
      max: 20,
...
Your summary or [none]:`;
      ...
      return (0, import_string2.getMessageContent)(summary.content);
    } catch (error) {
      console.error(error);
...
function getModelNames(service) {
  return service.getAllModels(import_types.ModelType.llm).concat("empty").map((m) => import_koishi.Schema.const(m));
}
...
var SearchManager = class {
  ...
  updateSchema(schema) {
    ...
    this.ctx.schema.set(
      "search-engine",
      import_koishi2.Schema.array(import_koishi2.Schema.union(this.schemas))
    );
  }
  ...
  async _reRankResults(query, results, limit) {
    ...
    const vectorStore = new import_vectorstores.MemoryVectorStore(embeddings);
...
var BingAPISearchProvider = class extends SearchProvider {
  ...
  static schema = import_koishi3.Schema.const("bing-api").i18n({
    "": "Bing (Azure API)"
  });
  name = "bing-api";
};
...
var BingWebSearchProvider = class extends SearchProvider {
  ...
  static schema = import_koishi4.Schema.const("bing-web").i18n({
    "": "Bing (Web)"
  });
  name = "bing-web";
};
...
var DuckDuckGoSearchProvider = class extends SearchProvider {
  ...
  async _getUrl(method, url, params) {
    for (let i = 0; i < 3; i++) {
      ...
      await (0, import_koishi5.sleep)(3e3);
    }
    return void 0;
  }
  /**
  ...
  static schema = import_koishi5.Schema.const("duckduckgo-lite").i18n({
    "": "DuckDuckGo (Lite)"
  });
...
var FreeGoogleSearchProvider = class extends SearchProvider {
  ...
  static schema = import_koishi6.Schema.const("free-google-api").i18n({
    "": "Free Google (API & Web)"
  });
...
var GoogleWebSearchProvider = class extends SearchProvider {
  ...
  static schema = import_koishi7.Schema.const("google-web").i18n({
    "": "Google (Web)"
  });
  name = "google-web";
};
...
var SearxNGSearchProvider = class extends SearchProvider {
  ...
  static schema = import_koishi8.Schema.const("searxng").i18n({
    "": "SearxNG"
  });
  name = "searxng";
};
...
var SerperSearchProvider = class extends SearchProvider {
  ...
  static schema = import_koishi9.Schema.const("serper").i18n({
    "": "Serper (Google API)"
  });
  name = "serper";
};
...
var TavilySearchProvider = class extends SearchProvider {
  ...
  static schema = import_koishi10.Schema.const("tavily").i18n({
    "": "Tavily (API)"
  });
  name = "tavily";
};
...
var WikipediaSearchProvider = class extends SearchProvider {
  ...
  static schema = import_koishi11.Schema.const("wikipedia").i18n({
    "": "Wikipedia"
  });
  name = "wikipedia";
};
...
var SearchTool = class extends import_tools2.Tool {
  ...
  _textSplitter = new import_textsplitters.RecursiveCharacterTextSplitter({
    chunkSize: 600,
    chunkOverlap: 100
  });
  async _call(arg) {
    ...
    return JSON.stringify(
      await this._reRankDocuments(
        (0, import_string3.getMessageContent)(fakeSearchResult.content),
        documents
      )
    );
  }
  ...
  async _reRankDocuments(query, documents) {
    if (this.embeddings === import_in_memory.emptyEmbeddings) {
    ...
    const vectorStore = new import_vectorstores2.MemoryVectorStore(this.embeddings);
...
var GENERATE_FAKE_SEARCH_RESULT_PROMPT = new import_prompts2.PromptTemplate({
      ...
      async createTool(params, session) {
        ...
        const browserTool = new PuppeteerBrowserTool(
          ...
          params.embeddings,
          {
            ...
            timeout: summaryType !== "quality" /* Quality */ ? 6 * import_koishi12.Time.second : 30 * import_koishi12.Time.second,
            idleTimeout: 3 * import_koishi12.Time.minute
          }
        );
...
async function createModel(ctx, model) {
  ...
  const [platform, modelName] = (0, import_count_tokens2.parseRawModelName)(model);
...
var Config4 = import_koishi12.Schema.intersect([
  import_chat.ChatLunaPlugin.Config,
  import_koishi12.Schema.object({
    searchEngine: import_koishi12.Schema.array(
      import_koishi12.Schema.union([
        import_koishi12.Schema.const("free-google-api").description(
          "Free Google (API & Web)"
        ),
        import_koishi12.Schema.const("bing-web").description("Bing (Web)"),
        import_koishi12.Schema.const("bing-api").description("Bing (API)"),
        import_koishi12.Schema.const("duckduckgo-lite").description(
          "DuckDuckGo (Lite)"
        ),
        import_koishi12.Schema.const("serper").description("Serper (Google)"),
        import_koishi12.Schema.const("tavily").description("Tavily (API)"),
        import_koishi12.Schema.const("google-web").description("Google (Web)"),
        import_koishi12.Schema.const("wikipedia").description("Wikipedia"),
        import_koishi12.Schema.const("searxng").description("SearxNG")
      ])
    ).default(["free-google-api"]).role("select"),
    topK: import_koishi12.Schema.number().min(2).max(50).step(1).default(5),
    puppeteerTimeout: import_koishi12.Schema.number().default(6e4),
    puppeteerIdleTimeout: import_koishi12.Schema.number().default(3e5),
    summaryType: import_koishi12.Schema.union([
      import_koishi12.Schema.const("speed"),
      import_koishi12.Schema.const("balanced"),
      import_koishi12.Schema.const("quality")
    ]).default("speed"),
    mulitSourceMode: import_koishi12.Schema.union([
      import_koishi12.Schema.const("average"),
      import_koishi12.Schema.const("total")
    ]).default("average"),
    summaryModel: import_koishi12.Schema.dynamic("model").default("empty"),
    searchThreshold: import_koishi12.Schema.percent().step(0.01).default(0.25),
    contextualCompression: import_koishi12.Schema.boolean().default(false)
  }),
  import_koishi12.Schema.object({
    serperApiKey: import_koishi12.Schema.string().role("secret"),
    serperCountry: import_koishi12.Schema.string().default("cn"),
    serperLocation: import_koishi12.Schema.string().default("zh-cn"),
    serperSearchResults: import_koishi12.Schema.number().min(2).max(20).default(10)
  }),
  import_koishi12.Schema.object({
    bingSearchApiKey: import_koishi12.Schema.string().role("secret"),
    bingSearchLocation: import_koishi12.Schema.string().default("zh-CN"),
    azureLocation: import_koishi12.Schema.string().default("global")
  }),
  import_koishi12.Schema.object({
    tavilyApiKey: import_koishi12.Schema.string().role("secret")
  }),
  import_koishi12.Schema.object({
    freeSearchBaseURL: import_koishi12.Schema.string().default(
      "https://search.dingyi222666.top"
    )
  }),
  import_koishi12.Schema.object({
    wikipediaBaseURL: import_koishi12.Schema.array(import_koishi12.Schema.string()).default([
      "https://en.wikipedia.org/w/api.php",
      "https://mzh.moegirl.org.cn/api.php"
    ]),
    maxWikipediaDocContentLength: import_koishi12.Schema.number().default(5e3)
  }),
  import_koishi12.Schema.object({
    searxngBaseURL: import_koishi12.Schema.string().default("https://paulgo.io")
  }),
  import_koishi12.Schema.object({
    searchPrompt: import_koishi12.Schema.string().role("textarea").default(
...
Output Language need same as user input language.`
    ...
    newQuestionPrompt: import_koishi12.Schema.string().role("textarea").default(
...
JSON Response:`
    ...
    searchFailedPrompt: import_koishi12.Schema.string().role("textarea").default(
...
Use same language as query. Suggest alternative search terms if possible.`
    ...
    contextualCompressionPrompt: import_koishi12.Schema.string().role("textarea").default(`Summarize the context based on the search action. Format in Markdown with citations. Return 'empty' if nothing relevant found.
