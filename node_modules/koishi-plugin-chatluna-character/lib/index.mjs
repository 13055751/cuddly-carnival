      ...
      if (el.type === "text") {
        if (el.attrs.code || el.attrs.span) {
          ...
          for (const text of splitSentence(
            ...
            canAppendAt() ? last().push(h.text(text)) : result.push([h.text(text)]);
          }
        } else {
        ...
        last() ? last().push(h.text(" "), el, h.text(" ")) : result.push([h.text(" "), el, h.text(" ")]);
...
function processTextMatches(rawMessage, useAt = true, markdownRender = true) {
  ...
  if (tokens.length === 0) {
    return {
      currentElements: markdownRender ? transform(rawMessage) : [h("text", { content: rawMessage })],
  ...
  for (const token of tokens) {
    ...
    switch (token.type) {
      ...
      case "message": {
        ...
        const children = token.children ? processTextMatches(token.content, useAt, markdownRender).currentElements : [h("text", { span: true, content: token.content })];
...
async function createChatLunaChain(ctx, model, systemPrompt, session) {
  ...
  const executor = AgentExecutor.fromAgentAndTools({
    tags: ["react-agent"],
    agent: llm.modelInfo.functionCall === true ? createOpenAIAgent({
      llm,
      tools,
      prompt: chatPrompt
...
async function setupModelPool(ctx, config) {
  const [platform, modelName] = parseRawModelName2(config.model);
  ...
  if (config.modelOverride?.length > 0) {
    for (const override of config.modelOverride) {
      modelPool[override.groupId] = (async () => {
        const [platform2, modelName2] = parseRawModelName2(override.model);
...
async function prepareMessages(messages, config, session, model, currentPreset, temp, stickerService, chain) {
  ...
  const humanMessage = new HumanMessage2(
    ...
    for (const message of messages) {
      if (message.images && message.images.length > 0) {
        for (const image of message.images) {
          const imageMessage = new HumanMessage2(
            `[image:${image.hash}]`
          );
  ...
  return formatCompletionMessages(
    [new SystemMessage2(formattedSystemPrompt)].concat(
...
async function handleStickerSending(session, config, parsedResponse, stickerService) {
  ...
  if (Math.random() < config.sendStickerProbability) {
    ...
    await sleep(random.int(500, 2e3));
...
__name(apply2, "apply");
var WINDOW_SIZE = 100;
var MAX_INTERVAL = Time.minute * 5;
var MIN_COOLDOWN_TIME = Time.second * 15;
...
import { h as h4, Service, Time as Time2 } from "koishi";
...
var Preset = class {
  ...
  async loadAllPreset() {
    ...
    this.ctx.schema.set(
      "character-preset",
      Schema2.union(
        this._presets.map((preset) => preset.name).concat("无").map((name2) => Schema2.const(name2))
      )
    );
...
var StickerService = class {
  ...
  _stickers = {};
  async init() {
    const sickerDir = path2.resolve(
      this._ctx.baseDir,
      "data/chathub/character/sticker"
    );
    try {
      await fs2.access(sickerDir);
    } catch (error) {
      await fs2.mkdir(sickerDir, { recursive: true });
      const dirname = __dirname?.length > 0 ? __dirname : fileURLToPath2(import.meta.url);
      await fs2.cp(
        path2.resolve(dirname, "../resources/sticker"),
        sickerDir,
        {
          recursive: true
        }
    ...
    const dirs = await fs2.readdir(sickerDir);
    for (const dirName of dirs) {
      const dir = path2.resolve(sickerDir, dirName);
      const stats = await fs2.stat(dir);
      if (stats.isDirectory()) {
        const stickers = await fs2.readdir(dir);
        this._stickers[dirName] = stickers.map(
          (sticker) => path2.resolve(dir, sticker)
        );
      }
    }
  ...
  async randomStickByType(type) {
    ...
    return h3.image(await readFile(sticker), `image/${getFileType(sticker)}`);
  }
  async randomStick() {
...
// src/service/message.ts
var MessageCollector = class extends Service {
  ...
  async broadcast(session) {
    ...
    const elements = session.elements ? session.elements : [h4.text(session.content)];
    ...
    groupArray = groupArray.filter((message2) => {
      return message2.timestamp == null || message2.timestamp >= now - Time2.hour;
    });
...
var Config = Schema3.intersect([
  Schema3.object({
    applyGroup: Schema3.array(Schema3.string()).description("应用到的群组"),
    maxMessages: Schema3.number().description("存储在内存里的最大消息数量").default(10).min(3).role("slider").max(100),
    disableChatLuna: Schema3.boolean().default(true).description("在使用此插件的群聊里，是否禁用 ChatLuna 主功能"),
    whiteListDisableChatLuna: Schema3.array(Schema3.string()).description(
      "在使用此插件时，不禁用 ChatLuna 主功能的群聊列表"
    )
  }).description("基础配置"),
  Schema3.object({
    model: Schema3.dynamic("model").default("").description("使用的模型"),
    modelOverride: Schema3.array(
      Schema3.object({
        groupId: Schema3.string().required().description("群组 ID"),
        model: Schema3.dynamic("model").default("").description("模型")
      })
    ).description("针对某个群的模型设置，会覆盖上面的配置"),
    maxTokens: Schema3.number().default(5e3).min(1024).max(42e3).description("聊天的最大 token 数"),
    image: Schema3.boolean().description(
      "是否允许输入图片（注意表情包也会输入，目前仅支持原生多模态的模型）"
    ).default(false),
    imageInputMaxCount: Schema3.number().default(3).min(1).max(15).description("最大的输入图片数量"),
    imageInputMaxSize: Schema3.number().default(3).min(1).max(20).description("最大的输入图片大小（MB）"),
    toolCalling: Schema3.boolean().description("是否启用工具调用功能").default(false),
    toolCallingPrompt: Schema3.string().description("工具调用的提示词").role("textarea").default(`你是一个智能助手，能够根据用户的意图调用相应的工具来提供帮助。并输出合理的 markdown 文档。
  ...
  `),
    toolCallingModel: Schema3.dynamic("model").default("").description("工具调用使用的模型")
  }).description("模型配置"),
  Schema3.object({
    isNickname: Schema3.boolean().description("允许 bot 配置中的昵称引发回复").default(true),
    isNickNameWithContent: Schema3.boolean().description(
      "是否允许在对话内容里任意匹配 bot 配置中的昵称来触发对话"
    ).default(false),
    isForceMute: Schema3.boolean().description(
    ...
    splitSentence: Schema3.boolean().description(
      "是否启用自分割发送消息 **注意请确保你的预设和模型在使用时支持自分割消息，否则请不要关闭**"
    ).default(true),
    markdownRender: Schema3.boolean().description(
      "是否启用 Markdown 渲染。关闭后可能会损失分割消息的精度"
    ).default(true),
    messageInterval: Schema3.number().default(14).min(0).role("slider").max(1e4).description("随机发送消息的间隔"),
    messageActivityScore: Schema3.number().default(0.6).min(0).max(1).role("slider").step(1e-5).description(
      "消息活跃度分数的阈值，当活跃度超过这个阈值则会发送消息"
    ),
    coolDownTime: Schema3.number().default(10).min(1).max(60 * 24).description("冷却发言时间（秒）"),
    typingTime: Schema3.number().default(440).min(100).role("slider").max(1500).description("模拟打字时的间隔（毫秒）"),
    largeTextSize: Schema3.number().default(300).min(100).max(1e3).description("大文本消息的判断阈值（字符数）"),
    largeTextTypingTime: Schema3.number().default(100).min(10).max(1500).description("大文本消息的固定打字间隔（毫秒）"),
    muteTime: Schema3.number().default(1e3 * 60).min(1e3).max(1e3 * 60 * 10 * 10).description("闭嘴时的禁言时间（毫秒）"),
    modelCompletionCount: Schema3.number().default(3).min(0).max(6).description("模型历史消息轮数，为 0 不发送之前的历史轮次"),
    sendStickerProbability: Schema3.number().default(0).min(0).max(1).role("slider").step(0.01).description("发送表情的概率（即将废弃，将制作新的表情系统插件）"),
    defaultPreset: Schema3.dynamic("character-preset").description("使用的伪装预设").default("煕")
  }).description("对话设置"),
  Schema3.object({
    configs: Schema3.dict(
      Schema3.object({
        maxTokens: Schema3.number().default(4e3).min(1024).max(2e4).description("使用聊天的最大 token 数"),
        isAt: Schema3.boolean().description("是否启用@").default(true),
        splitVoice: Schema3.boolean().description("是否分段发送语音").default(false),
        splitSentence: Schema3.boolean().description(
          "是否启用自分割发送消息 **注意请确保你的预设和模型在使用时支持自分割消息，否则请不要关闭**"
        ).default(true),
        ...
        messageInterval: Schema3.number().default(10).min(0).role("slider").max(1e4).description("随机发送消息的间隔"),
        messageActivityScore: Schema3.number().default(0.6).min(0).max(1).role("slider").step(1e-5).description(
          "消息活跃度分数的阈值，当活跃度超过这个阈值则会发送消息"
        ),
        toolCalling: Schema3.boolean().description("是否启用工具调用功能").default(false),
        ...
        imageInputMaxCount: Schema3.number().default(3).min(1).max(15).description("最大的输入图片数量"),
        imageInputMaxSize: Schema3.number().default(1024 * 1024 * 10).min(1024 * 1024 * 1).max(1024 * 1024 * 20).description("最大的输入图片大小（KB）"),
        coolDownTime: Schema3.number().default(10).min(1).max(60 * 24 * 24).description("冷却发言时间（秒）"),
        typingTime: Schema3.number().default(440).min(100).role("slider").max(1700).description("模拟打字时的间隔（毫秒）"),
        largeTextSize: Schema3.number().default(300).min(100).max(1e3).description("大文本消息的判断阈值（每段分句的字符数）"),
        largeTextTypingTime: Schema3.number().default(100).min(10).max(1500).description("大文本消息的模拟打字间隔（毫秒）"),
        muteTime: Schema3.number().default(1e3 * 60).min(1e3).max(1e3 * 60 * 10 * 10).description("闭嘴时的禁言时间（毫秒）"),
        modelCompletionCount: Schema3.number().default(3).min(0).max(6).description("模型历史消息轮数，为 0 不发送之前的历史轮次"),
        sendStickerProbability: Schema3.number().default(0).min(0).max(1).role("slider").step(0.01).description("发送表情的概率"),
        preset: Schema3.dynamic("character-preset").description("使用的伪装预设").default("煕")
      })
