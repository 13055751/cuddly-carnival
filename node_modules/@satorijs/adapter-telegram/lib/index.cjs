  ...
  const parseText = /* @__PURE__ */ __name((text, entities) => {
    ...
    for (const bp of Array.from(breakpoints).sort((a, b) => a - b)) {
      if (start < bp) {
        ...
        if (attr.includes("bold")) ele = (0, import_core.h)("b", {}, ele);
        if (attr.includes("italic")) ele = (0, import_core.h)("i", {}, ele);
        if (attr.includes("underline")) ele = (0, import_core.h)("u", {}, ele);
        if (attr.includes("strikethrough")) ele = (0, import_core.h)("s", {}, ele);
        if (attr.includes("code")) ele = (0, import_core.h)("code", {}, ele);
        if (attr.includes("pre")) ele = (0, import_core.h)("pre", {}, ele);
        if (attr.includes("spoiler")) ele = (0, import_core.h)("spl", {}, ele);
        if (attr.includes("mention")) ele = (0, import_core.h)("at", { name: content.slice(1) });
        if (url) ele = (0, import_core.h)("a", { href: url }, ele);
        if (user) ele = (0, import_core.h)("at", { id: user.id, name: user.username }, ele);
...
var TelegramMessageEncoder = class extends import_core2.MessageEncoder {
  ...
  async visit(element) {
    ...
    if (type === "text") {
      ...
      this.payload.caption += `<code>${attrs.content ? import_core2.h.escape(attrs.content) : children.toString()}</code>`;
...
var HttpServer = class extends import_core3.Adapter {
  ...
  async connect(bot) {
    ...
    this.ctx.server.post(path, async (ctx) => {
      ...
      if (!(bot2?.config?.token === token2)) return ctx.status = 403;
      ctx.body = "OK";
...
var TelegramBot = class _TelegramBot extends import_core5.Bot {
  ...
  async $getFileFromPath(filePath, extra) {
    if (this.server) {
