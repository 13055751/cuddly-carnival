...
var BaseConfig = import_koishi.Schema.intersect([
  ...
  import_koishi.Schema.union([
    import_koishi.Schema.object({
      defaultEngine: import_koishi.Schema.const("BERT-VITS2"),
      reference_audio: import_koishi.Schema.path().description("参考音频"),
      text_prompt: import_koishi.Schema.string().default("happy").description("融合文本语义"),
      prompt_text: import_koishi.Schema.string().hidden(true).default("happy").description("参考文本"),
      prompt_lang: import_koishi.Schema.string().default("auto").hidden(true).description("参考文本语言")
    }).description("BERT-VITS2 引擎配置"),
    import_koishi.Schema.object({
      defaultEngine: import_koishi.Schema.const("GPT-SOVITS"),
      reference_audio: import_koishi.Schema.path().description("参考音频，必填"),
      prompt_text: import_koishi.Schema.string().default("happy").description("参考文本，必填"),
      text_prompt: import_koishi.Schema.string().default("happy").hidden(true).description("融合文本语义"),
      prompt_lang: import_koishi.Schema.string().default("auto").description("参考文本语言")
...
__name(optionsToFormData, "optionsToFormData");
  ...
  constructor(ctx, config) {
    ...
    ctx.on("ready", async () => {
      ...
      if (config.reference_audio) {
        this.reference_audio = (0, import_fs.readFileSync)(config.reference_audio);
    ...
    ctx.command("切换语音 <input:text>", "更换语音角色，每个用户独立", { checkArgCount: true }).action(async ({ session }, input) => this.handleSwitch(session, input));
    ctx.command("say <input:text>", "vits语音合成").option("speaker", "-s <speaker:number>", { fallback: config.speaker_id }).option("lang", "-l <lang:string>").action(async ({ session, options }, input) => this.handleSay(session, options, input));
  }
...
${speaker_text}`;
  ...
  async handleSay(session, options, input) {
    ...
    const languageCodes = ["zh", "en", "fr", "ja", "ru", "de"];
    ...
    return await this.baseSay(result, engine, options.lang);
  }
  /**
   *
   * @param input 要转化的文本
   ...
   * @returns
   */
  async baseSay(option, engine, optionLang) {
    ...
    try {
      let options = {
        ...
        reference_audio: this.reference_audio ? new Blob([this.reference_audio], { type: this.reference_audio_mime }) : void 0
      };
      let formData = optionsToFormData(options);
